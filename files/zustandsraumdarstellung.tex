...
\begin{align*}
    ...
    \text{State space to TF:}\\
    &\Rightarrow C (sI-A)^{-1} B + D
\end{align*}
The inversion of $G(s)$ to find a state space description is not unique.
every system has one TF representation, if it is minimal.

\subsubsection{Controllable Canonical Form (for controller design)}
...
\subsubsection{Observable Canonical Form (for observer design: Pole placement)}
\begin{align*}
    A_o &= \begin{bmatrix}
        0 & 0 & \dots & -a_0 \\
        1 & 0 & \dots & -a_1 \\
...
initial state $x(0)$ into any another state $x(T)$ in finite time $T$.
It is controllable if the eigenvalues of $A-BK$, which are the poles of the closed-loop, can be placed arbitrarily, thus $P_c$ has full rank:
\[
    \rank(P_c) = \rank\left(
        \begin{bmatrix}
