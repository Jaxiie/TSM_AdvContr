...
\begin{minipage}{10cm}
    ...
    where
    \[
        K_{vf} = (C(sI-A+BK)^{-1}B)^{-1}
    \]
\end{minipage}
\hspace{0.5cm}
\begin{minipage}{8cm}
    \centering
    \includegraphics[width=\linewidth]{./bilder/statereg (2).png}
\end{minipage}
...
\[
    \det(\lambda I-A+BK) = (\lambda - \lambda_1)(\lambda - \lambda_2)...(\lambda - \lambda_n)
\]
Some considerations:
\begin{itemize}
...
Place as many poles as the number of states that the system has.
...
General notation:
\[
    J = f(K) = \int_{0}^{t_f} g(x,u)\, dt = \int_{0}^{t_f} x^{\mathsf{T}}Qx\, dt
\]
We define $H = A - BK$ and introduce a positive semidefinite weighting matrix $Q$.
To minimize the norm of the state variable $x$: $|x|^2 = x^{\mathsf{T}}x$, we get:
\begin{align*}
    J &= \int_0^{\infty} x^{\mathsf{T}} Q x \, dt = x^{\mathsf{T}}(0) P x(0) & \text{and} && (H^{\mathsf{T}}P + PH) &= -Q
\end{align*}

Design steps:
\begin{enumerate}
    \item Determine the matrix $P$, where $H$ and $Q$ are assumed to be known.
    \item Minimize $J = x^{\mathsf{T}}(0) P x(0)$ by adjusting system parameters.
\end{enumerate}

\subsection{Linear Quadratic Regulator (LQR)}
To consider the control energy of $u$, the performance index becomes
\[
    J = \int_{0}^{\infty} (x^{\mathsf{T}} Q x + u^{\mathsf{T}} R u)\, dt
\]
...
\[
    A^{\mathsf{T}} P + P A - P B R^{-1} B^{\mathsf{T}} P = -Q
\]

Design steps:
\begin{enumerate}
    \item Determine $P$, where $Q$ and $R$ have to be known (often $I$).
    \item Calculate $K = R^{-1} B^{\mathsf{T}} P$
\end{enumerate}

If the system is not fully controllable, lets hope the states that are not controllable are stable.
...
\subsection{LQI Linear Quadratic Integral}
\begin{minipage}{10cm}
    ...
    To calculate $K_1$ and $K_2$ at once, one defines the error $e = y-r$. Then:
    \[
        \underbrace{\dot{x}_{PI} =\begin{bmatrix}
        \dot{x} \\ \dot{x}_I
...
\begin{minipage}{8cm}
    \centering
    \includegraphics[width=8cm]{./bilder/statereg_int (2).png}
\end{minipage}
...
e.g. by LQR or pole placement.
    ...
    \[
        u = K_{PI}\cdot x_{PI}= \begin{bmatrix}\hat{K}&K_I\end{bmatrix}\cdot x_{PI} 
    \]
...
\begin{minipage}{10cm}
    ...
    \begin{align*}
        ...
        P_c &\Leftrightarrow P_o^{\mathsf{T}} &
        B &\Leftrightarrow C^{\mathsf{T}} \\
        A &\Leftrightarrow A^{\mathsf{T}} &
        K &\Leftrightarrow H^{\mathsf{T}}
    \end{align*}
    The calculation of the optimal $H$ is called the \emph{Linear Quadratic Gaussian (LQG)} problem and is
    \begin{enumerate}
        \item $AP + PA^{\mathsf{T}} - PC^{\mathsf{T}}R^{-1}CP = -Q$ which deliveres $P$
        \item $H = (R^{-1}CP)^{\mathsf{T}} = PC^{\mathsf{T}}R^{-1}$
    \end{enumerate}
...
\begin{minipage}{8cm}
    \centering
    \includegraphics[width=8cm]{./bilder/observer (2).png}
\end{minipage}
...
\begin{enumerate}
    ...
    \item Do LQG design with $Q = \rho B B^{\mathsf{T}}$ and $R = I$, resulting in an observer gain $H$.
\end{enumerate}
...
\]
With $Q =\rho C^{\mathsf{T}} C$ and $R=I$, the LQR design design recovers the LQG loop transfer function,
which leads to
\[
...
\textbf{Kalman filter} &                               &\textbf{LQR/LQG Recovery} &                \\
$\boldsymbol{Q} = \rho\boldsymbol{BB^{\mathsf{T}}}$ &  & $\boldsymbol{Q} = \boldsymbol{C}^{\mathsf{T}}I\boldsymbol{C}$  \\
$\boldsymbol{R} = \boldsymbol{\mu \beta}$           &  & $\boldsymbol{R} = \rho\boldsymbol{I}$                          \\
$\mu > 0$                                           &  & $\rho > 0$     \\
$\beta \ge 1$ typical $2-4$            &                             
\end{tabularx}