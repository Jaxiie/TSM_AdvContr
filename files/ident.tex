...
For creation of Bode plot apply testsignal, wait for transiant to decay and measure $B \& \varphi$,
calculate the gain and insert date in Plot.
...
The sample time $T$ and the number of samples $N$ are chosen to be $T \cdot N = \frac{2\pi}{\omega_0}l$.
Then the $B_c$ and $B_s$ which minimize the quadratic cost function reduce to
\begin{align*}
    B_c &= \frac{2}{N}\sum_{k=0}^{N-1} y(kT) \cos\left(\frac{2\pi lk}{N}\right) &
    B_s &= \frac{2}{N}\sum_{k=0}^{N-1} y(kT) \sin\left(\frac{2\pi lk}{N}\right)
\end{align*}
...
\[
    \operatorname{DFT}\{f(kT)\} = F_n = \sum_{k=0}^{N-1} f(kT) e^{-j\frac{2\pi nk}{N}}
\]
where $F_n = F\left(\frac{2\pi n}{NT}\right)$ is the frequency component at the frequency $\omega = \frac{2\pi n}{NT}$.
Using e.g. white noise or chirp signals as input signal $u$, one can calculate the frequency response by
\[
    G_d(e^{j\omega_0 T}) = \frac{\operatorname{DFT}\{y(kT)\}}{\operatorname{DFT}\{u(kT)\}} = \frac{B_s + jB_c}{A} = \frac{B}{A}e^{j\varphi}
\]
...
if $v=0$ $\hat{g}_d=\frac{y(kT)}{A}$ else $\hat{g}_d =\frac{y(kT)-v(kT)}{A}$
...
The q-operator shift signal by one time step $y(k-1)=q^{-1}y(k)$
...
Physical parameters are \emph{not} necessairly identifiable.
...
for $y=f(x)=ax^2+b$ with measured \(x\) and \(y\):
...
\[
    y=\begin{bmatrix}y_{1} \\
         ...
         \vdots
    \end{bmatrix}=
    \begin{bmatrix}ax_{1}^{2}+b \\
         ax_{2}^{2}+b \\
         \vdots\\
         \vdots
    \end{bmatrix}=
    \underbrace{ \begin{bmatrix}x_{1}^{2} & 1 \\
         x_{2}^{2} & 1 \\ 
         x_{3}^{2} & 1 \\ 
         \vdots & \vdots
    \end{bmatrix} }_{ \Phi }
    \underbrace{ \begin{bmatrix}a \\
         b\end{bmatrix} }_{ \Theta }
\]
...
(typical values: $0.9 < \lambda < 0.98$) and entries \(<1\) are less important
\[
...
Controller and data aquisition is discrete and has usually the same sampling time.
The smaller the better, but too small can lead to numerical problems
\paragraph{Identification of Open-Loop System} give an random input and measure output, fails at unstable system, is simpler
\paragraph{Identification of Closed-Loop Systems} A plant might be unstable or
in use for production (time-varying), or it might contain some built-in feedback mechanisms.
Then the plant has to be identified in a closed loop.

\subsection{System Identification Matlab}
Matlab command: \dq systemIdentification\dq{} 

load Data $\rightarrow$ preprocess Data $\rightarrow$ estimate Model

\begin{itemize}
    \item preprocess: remove mean and select range (faster: Quickstart)
    \item estimate: choose Model (State Space, Transferfunction)
\end{itemize}

overfitting analysis with pole zero plot, when ther are pole zero cancellations (zeros and poles at same place) its a sign of overfitting